defmodule YamlixTest do
  ...
  test "it dumps spec symbols" do
    map = %{
      "regex" => "quotation marks",
      "examples" => [
        "- ([\"'])(?:(?=(\\\\?))\\2.)*?\\1",
        "- /('[^']*')|(\"[^\"]*\")/g",
        "- (?<=[\"'])[^\"']+"
    ...
    assert content = Yamlix.dump(map, wrap: false, multiline_fields: ["examples"])
  ...
  test "it dumps lists of strings as literal content" do
    assert Yamlix.dump(["one", "two"], literal: true) == """
           ---\s
           one
           two
           ...
